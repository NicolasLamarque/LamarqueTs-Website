...
// ⚠️ IMPORTANT: Charger dotenv EN PREMIER avant tout import
import { config } from 'dotenv';
config(); // Charge le .env MAINTENANT
...
async function seed() {
  ...
  console.log("📍 Toutes les vars d'env:", Object.keys(process.env).filter(k => k.includes('DATA')));
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO users (username, mail, password, role, is_active) 
       VALUES ($1, $2, $3, $4, $5) 
       RETURNING *`,
